        ...
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:'12px', padding:'16px 24px', position:'sticky', top:'68px', zIndex:10, background:'rgba(6,13,42,0.7)', backdropFilter:'blur(10px)', borderBottom:'1px solid var(--bf)' }}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:'10px' }}>
            ...
            <button
              ...
              style={{ display:'none', padding:'8px 14px', borderRadius:'50px', fontSize:'11px', fontWeight:600, cursor:'pointer', background: showFilters ? 'rgba(212,160,23,0.12)' : 'rgba(255,255,255,0.04)', border:'1px solid var(--bf)', color:'var(--gold3)', fontFamily:'Raleway,sans-serif', whiteSpace:'nowrap' }}
            >
              🔧 {showFilters ? 'Hide Filters' : 'Filters'}{Object.keys(filters).length > 0 ? ` (${Object.keys(filters).filter(k => k !== 'search').length})` : ''}
            </button>
