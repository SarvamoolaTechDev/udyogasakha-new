    ...
    <section style={{ padding:'48px 4%' }}>
      <div style={{ marginBottom:'28px' }}>
        <h1 style={{ fontFamily:'Cinzel,serif', fontSize:'clamp(20px,3vw,32px)', fontWeight:700, color:'#fff', margin:0 }}>
          Audit <span style={{ background:'linear-gradient(135deg,var(--gold),var(--gold3))', WebkitBackgroundClip:'text', WebkitTextFillColor:'transparent', backgroundClip:'text' }}>Log</span>
        </h1>
        <p style={{ fontSize:'12px', color:'var(--muted)', marginTop:'4px' }}>Immutable record of all platform state changes. Read-only.</p>
      </div>
      ...
      <div className="gc" style={{ padding:'20px', marginBottom:'18px' }}>
        ...
        <div style={{ display:'flex', gap:'8px', marginBottom:'16px', flexWrap:'wrap' }}>
          {(['recent','entity','actor'] as Mode[]).map(m => (
            <button key={m} onClick={()=>{ setMode(m); setSubmitted(false); setPage(1); }} style={{ padding:'7px 16px', borderRadius:'50px', fontSize:'11px', fontWeight:600, cursor:'pointer', border:'1px solid', fontFamily:'Raleway,sans-serif', transition:'all 0.2s', background: mode===m ? 'rgba(212,160,23,0.12)' : 'transparent', borderColor: mode===m ? 'var(--border)' : 'var(--bf)', color: mode===m ? 'var(--gold3)' : 'var(--muted)' }}>
        ...
        {mode === 'entity' && (
          <div style={{ display:'flex', gap:'10px', flexWrap:'wrap', alignItems:'flex-end' }}>
            ...
            <div style={{ flex:1, minWidth:'220px' }}>
              ...
              <input value={entityId} onChange={e=>setEntityId(e.target.value)} className="fi" placeholder="xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx" style={{ fontFamily:'monospace', fontSize:'12px' }} />
            </div>
            <button onClick={submit} disabled={!entityId.trim()} className="btn-gold" style={{ padding:'11px 20px', borderRadius:'50px', border:'none', cursor:'pointer', fontSize:'12px', opacity:entityId.trim()?1:0.4, alignSelf:'flex-end' }}>
              Search
            </button>
          </div>
        )}
        ...
        {mode === 'actor' && (
          <div style={{ display:'flex', gap:'10px', flexWrap:'wrap', alignItems:'flex-end' }}>
            <div style={{ flex:1, minWidth:'220px' }}>
              ...
              <input value={actorId} onChange={e=>setActorId(e.target.value)} className="fi" placeholder="xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx" style={{ fontFamily:'monospace', fontSize:'12px' }} />
            </div>
            <button onClick={submit} disabled={!actorId.trim()} className="btn-gold" style={{ padding:'11px 20px', borderRadius:'50px', border:'none', cursor:'pointer', fontSize:'12px', opacity:actorId.trim()?1:0.4, alignSelf:'flex-end' }}>
              Search
            </button>
          </div>
        )}
      ...
      <div className="gc" style={{ overflow:'hidden' }}>
        <div className="table-scroll">
          <table className="rt">
            <thead>
              <tr>{['Timestamp','Action','Entity','Entity ID','Actor','Metadata'].map(h=><th key={h}>{h}</th>)}</tr>
            </thead>
            <tbody>
              {q.isLoading ? (
                Array.from({ length: 8 }).map((_,i) => <SkeletonRow key={i} cols={6} />)
              ) : entries.length === 0 ? (
                <tr><td colSpan={6} style={{ textAlign:'center', padding:'32px', color:'var(--muted)' }}>
                ...
                entries.map((e: any) => (
                  <tr key={e.id}>
                    <td style={{ fontSize:'10px', color:'var(--muted)', whiteSpace:'nowrap' }}>
                      ...
                      <span style={{ color:'var(--faint)' }}>{new Date(e.ts).toLocaleTimeString('en-IN',{ hour:'2-digit', minute:'2-digit', second:'2-digit' })}</span>
                    </td>
                    <td>
                    ...
                    <td style={{ fontSize:'10px', color:'var(--muted)', fontFamily:'monospace' }}>{e.entityId?.slice(-8)}</td>
                    <td style={{ fontSize:'10px', color:'var(--muted)', fontFamily:'monospace' }}>
                      {e.actorEmail || (e.actorId ? e.actorId.slice(-8) : '—')}
                    </td>
                    <td style={{ fontSize:'10px', color:'var(--muted)', maxWidth:'160px', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>
