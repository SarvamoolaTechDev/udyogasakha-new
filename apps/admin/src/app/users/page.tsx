              ...
              {isLoading ? (
                ...
                users.map((u:any)=>(
                  <tr key={u.id}>
                    ...
                    <td style={{ fontSize:'11px', color:'var(--muted)', textAlign:'center' }}>{u.profiles?.length??'—'}</td>
                    <td style={{ fontSize:'11px', color:'var(--muted)' }}>{u.createdAt ? new Date(u.createdAt).toLocaleDateString('en-IN',{ day:'numeric', month:'short', year:'numeric' }) : '—'}</td>
                    <td><Link href={`/users/${u.id}`} style={{ fontSize:'10px', color:'var(--gold3)', textDecoration:'none', fontWeight:600 }}>View →</Link></td>
                  </tr>
                ))
              )}
            </tbody>
